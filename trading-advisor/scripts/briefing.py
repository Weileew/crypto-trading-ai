...
def _liquidity_multiplier(c: dict) -> float:
    ...
    depth = (d or {}).get("depth_within_1pct") or c.get("tokocrypto_depth_usd", 0)
...
def render_compact_briefing(markets, global_data, fng, assets, visuals=False, enhanced=False, orchestrator=False):
    ...
    funnel_lines = _funnel_summary(markets, top=opps, score_threshold=_thresh_from_calibrations())
    if funnel_lines:
        ...
        try:
            ...
            if os.path.exists(_pfolio_path):
                ...
                if _positions:
                    ...
                    txt.append(f"  Equity: ${_equity:,.2f} · Cash: ${_cash:,.2f} · Positions: {len(_positions)}")
                    txt.append("")
        except Exception:
            pass
    txt += [
        ...
        for idx, c in enumerate(opps, 1):
            ...
            if isinstance(_depth_data, dict):
                ...
                if _liq_spread < 5 and _liq_total > 500_000:
                    _liq_grade = "A 🟢"
                elif _liq_spread < 15 and _liq_total > 100_000:
                    _liq_grade = "B 🟡"
                elif _liq_spread < 30:
                    _liq_grade = "C 🟠"
                else:
            ...
            if paper:
                ...
                # Word-boundary truncation at 140 chars
                _trim = finding[:140].strip()
                if len(finding) > 140:
